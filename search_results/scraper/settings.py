from __future__ import unicode_literals
# Scrapy settings for search_results project
#
# For simplicity, this file contains only the most important settings by
# default. All the other settings are documented here:
#
#     http://doc.scrapy.org/topics/settings.html
#

import os, sys

PROJECT_ROOT = os.path.abspath(os.path.dirname(__file__))
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "zs_project.settings")
sys.path.insert(0, os.path.join(PROJECT_ROOT, "../../..")) #only for zs


BOT_NAME = 'search_results'


#Setting LOG_STDOUT to True will prevent Celery scheduling to work, 2017-06-06
#If you know the cause or a fix please report on GitHub
LOG_STDOUT = False
LOG_LEVEL = 'INFO'


SPIDER_MODULES = ['dynamic_scraper.spiders', 'search_results.scraper',]
USER_AGENT = '{b}/{v}'.format(b=BOT_NAME, v='1.0')

ITEM_PIPELINES = {
    'dynamic_scraper.pipelines.DjangoImagesPipeline': 200,
    'dynamic_scraper.pipelines.ValidationPipeline': 400,
    'search_results.scraper.pipelines.DjangoWriterPipeline': 800,
}

IMAGES_STORE = os.path.join(PROJECT_ROOT, '../thumbnails')

IMAGES_THUMBS = {
    'medium': (50, 50),
    'small': (25, 25),
}

DSCRAPER_IMAGES_STORE_FORMAT = 'ALL'

DSCRAPER_LOG_ENABLED = True
DSCRAPER_LOG_LEVEL = 'ERROR'
DSCRAPER_LOG_LIMIT = 5